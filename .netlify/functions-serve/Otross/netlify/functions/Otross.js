...
var Otross_exports = {};
__export(Otross_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(Otross_exports);
...
var TIMEOUT_MS = 9e3;
var PAGE_SIZE = 12;
var CLOUDINARY_FETCH_PREFIX = null;
var FEEDS = {
  ai: [
  ...
  build: [
    // Arquitectura
    { url: "https://www.plataformaarquitectura.cl/cl", cat: "Arquitectura" },
    // HTML → OG
    // Construcción sostenible (WP → RSS)
    { url: "https://www.construible.es/feed", cat: "Construcci\xF3n" },
...
var decodeHtml = (s = "") => String(s).replace(/&amp;/g, "&").replace(/&quot;/g, '"').replace(/&#39;/g, "'");
var tryGet = (obj, ...paths) => {
  for (const p of paths) {
    const v = p.split(".").reduce((a, k) => a ? a[k] : void 0, obj);
    if (v) return v;
  }
  return null;
};
var isSpanishishUrl = (u = "") => {
  try {
    const { hostname, pathname } = new URL(u);
    return /\.(es|mx|cl|ar|pe)$/i.test(hostname) || hostname.startsWith("es.") || /\/es(\/|$)/i.test(pathname) || /plataformaarquitectura|archdaily/i.test(hostname);
  } catch {
    return false;
  }
};
...
var normalizeItem = async (item, sourceName, categoryHint) => {
  ...
  const rawDesc = item.description || tryGet(item, "content:encoded") || tryGet(item, "summary") || "";
  const summary = decodeHtml(String(rawDesc).replace(/<[^>]+>/g, "")).slice(0, 260);
  let image = tryGet(item, "media:content.@_url") || tryGet(item, "media:thumbnail.@_url") || tryGet(item, "enclosure.@_url") || tryGet(item, "enclosure.url") || tryGet(item, "image.@_href") || tryGet(item, "image.url") || null;
  if (!image) {
...
var parseFeedOrPage = async (url, categoryHint) => {
  const out = [];
  ...
  for (const a of anchors.slice(0, 12)) {
    const image = "/Resource/Logo/logo.png";
    out.push({
      title: a.title,
      url: a.link,
      image,
...
var AD_IMAGE = "https://images.pexels.com/photos/8000532/pexels-photo-8000532.jpeg?auto=compress&cs=tinysrgb&w=1200&h=675&dpr=1";
var buildAdCard = () => ({
  ...
  summary: "\xBFTienes una idea? Dise\xF1amos y lanzamos tu aplicaci\xF3n web o m\xF3vil con IA.",
  category: "Anuncio"
});
var roundRobin = (buckets, limit) => {
  const out = [];
  const arrs = buckets.map((b) => [...b]);
  let added = true;
...
var handler = async () => {
  try {
    ...
    let all = settled.filter((r) => r.status === "fulfilled").flatMap((r) => r.value).filter((n) => n && n.title && n.url);
    ...
    for (const n of all) {
      const cat = bucketsByCat[n.category] ? n.category : "Construcci\xF3n";
    ...
    const slice = roundRobin(INTERLEAVE_ORDER, Math.max(0, PAGE_SIZE - 1));
...
//# sourceMappingURL=Otross.js.map
